...
<tr><td><b>Id</b></td><td><CopyableCode code="google.ces.operations" /></td></tr>
</tbody></table>
...
```sql
DELETE FROM google.ces.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required
;
...
```sql
EXEC google.ces.operations.cancel 
