...
<tr><td><b>Id</b></td><td><CopyableCode code="google.cloudnumberregistry.operations" /></td></tr>
</tbody></table>
...
metadata,
response
FROM google.cloudnumberregistry.operations
...
```sql
DELETE FROM google.cloudnumberregistry.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required
;
...
```sql
EXEC google.cloudnumberregistry.operations.cancel 
