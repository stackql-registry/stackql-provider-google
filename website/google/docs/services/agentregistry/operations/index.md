...
<tr><td><b>Id</b></td><td><CopyableCode code="google.agentregistry.operations" /></td></tr>
</tbody></table>
...
metadata,
response
FROM google.agentregistry.operations
...
```sql
DELETE FROM google.agentregistry.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required
;
...
```sql
EXEC google.agentregistry.operations.cancel 
