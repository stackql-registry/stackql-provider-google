...
<tr><td><b>Id</b></td><td><CopyableCode code="google.hypercomputecluster.operations" /></td></tr>
</tbody></table>
...
metadata,
response
FROM google.hypercomputecluster.operations
...
```sql
DELETE FROM google.hypercomputecluster.operations
WHERE projectsId = '{{ projectsId }}' --required
AND locationsId = '{{ locationsId }}' --required
AND operationsId = '{{ operationsId }}' --required
;
...
```sql
EXEC google.hypercomputecluster.operations.cancel 
