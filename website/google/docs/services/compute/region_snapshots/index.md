...
Creates, updates, deletes, gets or lists a <code>region_snapshots</code> resource.

## Overview
<table><tbody>
<tr><td><b>Name</b></td><td><CopyableCode code="region_snapshots" /></td></tr>
<tr><td><b>Type</b></td><td>Resource</td></tr>
<tr><td><b>Id</b></td><td><CopyableCode code="google.compute.region_snapshots" /></td></tr>
</tbody></table>
...
<tr>
    ...
    <td><a href="#parameter-project"><code>project</code></a>, <a href="#parameter-region"><code>region</code></a>, <a href="#parameter-snapshot"><code>snapshot</code></a></td>
    <td></td>
...
<tr>
    ...
    <td>Retrieves the list of Snapshot resources contained within<br />the specified region.</td>
</tr>
<tr>
    ...
    <td>Creates a snapshot in the specified region using the data included<br />in the request.</td>
</tr>
<tr>
...
<tr>
    ...
    <td><a href="#parameter-requestId"><code>requestId</code></a></td>
    <td>Sets the labels on a regional snapshot. To learn more about labels, read<br />the Labeling Resources<br />documentation.</td>
</tr>
</tbody>
</table>
...
storageBytesStatus,
storageLocations
FROM google.compute.region_snapshots
...
Retrieves the list of Snapshot resources contained within<br />the specified region.
...
Creates a snapshot in the specified region using the data included<br />in the request.

```sql
INSERT INTO google.compute.region_snapshots (
data__chainName,
data__description,
data__guestFlush,
...
- name: region_snapshots
  props:
    - name: project
      value: "{{ project }}"
      description: Required parameter for the region_snapshots resource.
    - name: region
      value: "{{ region }}"
      description: Required parameter for the region_snapshots resource.
    - name: chainName
...
```sql
DELETE FROM google.compute.region_snapshots
WHERE project = '{{ project }}' --required
AND region = '{{ region }}' --required
AND snapshot = '{{ snapshot }}' --required
...
Sets the labels on a regional snapshot. To learn more about labels, read<br />the Labeling Resources<br />documentation.

```sql
EXEC google.compute.region_snapshots.set_labels 
