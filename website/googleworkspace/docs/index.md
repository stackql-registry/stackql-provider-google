...
WHERE q = "modifiedTime > '2026-01-01T00:00:00' and trashed = false";
```
...
```sql
SELECT
  json_extract(storageQuota, '$.limit') AS quota_bytes,
  json_extract(storageQuota, '$.usage') AS used_bytes,
  json_extract(user, '$.emailAddress') AS user_email
